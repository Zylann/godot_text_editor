...
signal scroll_offset_changed(v)
signal wrap_count_changed(c)
...
func _scroll(delta):
	...
	emit_signal("scroll_offset_changed", _smooth_scroll_offset)
	update()
...
func _set_text(text):
	...
	emit_signal("wrap_count_changed", len(_wraps))
...
func get_visible_lines_count():
	var line_height = int(_font.get_height())
	return int(rect_size.y) / line_height
...
func _draw():
	...
	var visible_lines = get_visible_lines_count()
